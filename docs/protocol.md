...
### Local trial: ZIP double-click

`archive` also accepts `op: "extract-here"`, with absolute `path` and `dest` equal to the archive's containing directory. It uses the existing sandboxed extraction and verified completion, then publishes complete top-level entries with no-replace renames. Any pre-existing top-level file, folder or dangling symlink aborts the whole publication. A concurrent collision stops publication and reports how many entries already completed; those entries remain intact. The original archive is retained.

ZIP double-click in list/grid/columns uses this operation. Search results keep reveal behavior; Enter keeps the archive preview. The existing Extract action continues to create a named subfolder.
