...
// Preserve trial discovery: USB bridges inherit externality; mounted leaves and unmounted internal
// partitions remain browsable. The system disk is excluded above; swap is not a mountable place.
function collectVolumes(nodes, model, external, out) {
    for (var i = 0; i < nodes.length; i++) {
        var n = nodes[i]
        var kids = n.children || []
        // Only the disk carries a product name, so it is passed down to its own partitions.
        var own = n.model ? String(n.model) : model
        var removable = external || isExternal(n)
        ...
        if (n.name && kids.length === 0 && (removable || mountOf(n).length > 0 || (n.type === "part" && (n.mountpoints || []).every(function (p) { return !p }))))
            out.push(volumeRow(n, own, removable))
        collectVolumes(kids, own, removable, out)
    }
}
...
function volumeRow(n, model, removable) {
    ...
    return { kind: "volume", label: label, device: devicePath(n), path: path, mounted: path.length > 0,
             removable: removable, size: deviceBytes(n.size) }
}
...
function isExternal(n) {
    return n.rm === true || String(n.tran || "").toLowerCase() === "usb"
        || /(^|:)usb(:|$)/.test(String(n.subsystems || "").toLowerCase())
}
