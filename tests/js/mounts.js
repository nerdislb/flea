...
function run(check) {
    ...
    var spare = '{"blockdevices":['
              ...
              + ']}'
    check("an unmounted internal partition remains available", Devices.parseDevices(spare).length, 2)
    ...
    var bridge = '{"blockdevices":['
               + '{"name":"sda","label":null,"mountpoints":[null],"rm":false,"hotplug":true,"tran":"usb","subsystems":"block:scsi:usb:pci","size":"931.5G","type":"disk","model":"WDC WD10JMVW-11AJGS4",'
               + '"children":[{"name":"sda1","label":"Hardsk Ario","mountpoints":["/mnt/Hardsk-Ario"],"rm":false,"hotplug":false,"tran":null,"subsystems":"block:scsi:pci","size":"931.5G","type":"part","model":null}]},'
               + '{"name":"nvme0n1","label":null,"mountpoints":[null],"rm":false,"hotplug":false,"tran":"nvme","subsystems":"block:nvme:pci","size":"476.9G","type":"disk","model":"SKHynix_HFS512GEJ9X164N","children":[{"name":"nvme0n1p1","mountpoints":["/"],"type":"part"}]}'
               + ']}'
    var b = Devices.parseDevices(bridge)
    check("a USB bridge with rm=false still yields its volume", b.length, 2)
    check("the internal row stays on the NVMe, not the USB disk", b[0].device, "/dev/nvme0n1")
    check("the bridged volume takes the filesystem label", b[1].label, "Hardsk Ario")
    check("the bridged volume carries its device node for gio", b[1].device, "/dev/sda1")
    ...
    var sataHotplug = '{"blockdevices":['
               + '{"name":"sda","label":null,"mountpoints":[null],"rm":false,"hotplug":true,"tran":"sata","subsystems":"block:scsi:pci","size":"1T","type":"disk","model":"Internal SATA",'
               + '"children":[{"name":"sda1","label":null,"mountpoints":["/"],"rm":false,"hotplug":false,"tran":"sata","subsystems":"block:scsi:pci","size":"1T","type":"part","model":null}]}'
               + ']}'
    var h = Devices.parseDevices(sataHotplug)
    ...
    var internalMedia = '{"blockdevices":['
              + '{"name":"nvme0n1","label":null,"mountpoints":[null],"rm":false,"size":"238.5G","type":"disk","model":"KBG40ZNS256G",'
              + '"children":[{"name":"nvme0n1p1","label":null,"mountpoints":["/"],"rm":false,"size":"2G","type":"part","model":null}]},'
              + '{"name":"sda","label":null,"mountpoints":[null],"rm":false,"size":"931.5G","type":"disk","model":"WDC WD10SPZX-75Z10T3",'
              + '"children":[{"name":"sda1","label":null,"mountpoints":["/run/media/gm/3a970f8f-aa74-4889-a80f-10f611ac8623"],"rm":false,"size":"931.5G","type":"part","model":null}]}'
              + ']}'
    var hdd = Devices.parseDevices(internalMedia, "/home/gm")
    ...
    check("a removable volume is tagged removable", Devices.parseDevices(live, "/home/gm")[1].removable, true)
    check("a /boot partition contributes no row", hdd.map(function (e) { return e.device }).join(","), "/dev/nvme0n1,/dev/sda1")
    check("device discovery does not depend on a home path", Devices.parseDevices(internalMedia).length, 2)

    // Unmounted and internal: the state an HDD sits in after an unmount, with an arrow to mount it.
    var internalIdle = '{"blockdevices":[{"name":"sda","label":null,"mountpoints":[null],"rm":false,"size":"931.5G","type":"disk","model":"WDC WD10SPZX-75Z10T3",'
              + '"children":[{"name":"sda1","label":null,"mountpoints":[null],"rm":false,"size":"931.5G","type":"part","model":null}]}]}'
    var hddIdle = Devices.parseDevices(internalIdle, "/home/gm")
    check("an unmounted internal partition is one row without an invented system disk", hddIdle.length, 1)
    check("it reads as unmounted with no path yet", hddIdle[0].mounted + "|" + hddIdle[0].path, "false|")
    check("it still carries the device node its mount needs", hddIdle[0].device, "/dev/sda1")
    check("it is tagged not removable", hddIdle[0].removable, false)

    // Containers and system states never become rows on their own.
    var luksDisk = '{"blockdevices":[{"name":"nvme0n1","label":null,"mountpoints":[null],"rm":false,"size":"238.5G","type":"disk","model":"KBG40ZNS256G",'
              + '"children":[{"name":"nvme0n1p2","label":null,"mountpoints":[null],"rm":false,"size":"236.5G","type":"part","model":null,'
              + '"children":[{"name":"root","label":null,"mountpoints":["/"],"rm":false,"size":"236.5G","type":"crypt","model":null}]}]}]}'
    check("a LUKS container with a stacked child is not a row", Devices.parseDevices(luksDisk, "/home/gm").length, 1)
